...
def build_parser() -> argparse.ArgumentParser:
    ...
    outline_init = sub.add_parser("outline-init", help="create project-owned story outline and first volume plan")
    outline_init.add_argument("--logline", required=True)
    outline_init.add_argument("--theme", default="")
    outline_init.add_argument("--volume-title", required=True)
    outline_init.add_argument("--chapter-start", type=int, default=1)
    outline_init.add_argument("--chapter-end", type=int, required=True)
    outline_init.add_argument("--core-conflict", required=True)
    outline_init.add_argument("--climax", required=True)
    outline_init.add_argument("--character", action="append", default=[])
    outline_init.add_argument("--global-rule", action="append", default=[])

    outline_revise = sub.add_parser("outline-revise", help="apply explicit author outline revisions from JSON")
    outline_revise.add_argument("--revision-file", required=True)

    outline_plan = sub.add_parser("plan-from-outline", help="create chapter contract from story_outline.json")
    outline_plan.add_argument("--chapter", type=int, required=True)
    ...
    draft_note.add_argument("--strict", action="store_true", help="fail if analysis evidence/manifest is incomplete")
    draft_note.add_argument(
        "--min-evidence-count",
        type=int,
        default=1,
        help="minimum evidence items required when --strict is used",
    )
    ...
    nl = sub.add_parser("nl", help="execute an agent_writer workflow from a Chinese natural-language request")
    nl.add_argument("--request", required=True)
    nl.add_argument("--dry-run", action="store_true")
    nl.add_argument("--allow-commit", action="store_true")
    ...
    if args.command == "outline-init":
        _print_json(
            create_story_outline(
                ...
                climax=args.climax,
                major_characters=args.character,
                global_rules=args.global_rule,
            )
        )
        return 0
    if args.command == "outline-revise":
        _print_json(revise_story_outline(root, revision_file=Path(args.revision_file)))
        return 0
    if args.command == "plan-from-outline":
        _print_json(plan_chapter_from_outline(root, chapter_number=args.chapter))
        return 0
